...
[下载最新的已签名、已公证 DMG](https://github.com/zuuzii-org/llm-pulse/releases/latest)。LLM Pulse 2.5.0 要求 macOS 14 或更高版本，同时支持 Apple Silicon 和 Intel Mac。

v2.5.0 Release 附件包括：

- `LLM-Pulse-2.5.0.dmg`
- `LLM-Pulse-2.5.0.dmg.sha256`

将两个文件放在同一目录，然后执行以下命令验证下载内容：

```bash
shasum -a 256 -c LLM-Pulse-2.5.0.dmg.sha256
```

v1.0.0 用户需要先手动安装一次 v1.1.0，因为 v1.0.0 尚未包含应用内更新。v1.1–v1.3 用户应先安装并启动 v1.4，再升级到 v2.5.0。
...
- **只关注进行中的工作。** 面板显示正在运行、等待授权和等待回答的任务；完成与失败通过通知送达，不再堆积成列表。
...
让 LLM Pulse 与 Codex Desktop 同时运行即可。菜单栏上方橙色数字为等待你确认的任务、下方蓝色数字为正在执行的任务，右侧边栏逐项显示每个进行中任务的状态、项目、持续时间、token 用量和活跃 Agent 总数。
...
它是根据 Codex 提供的已用百分比换算出的剩余百分比，并附带每周重置时间，不是绝对 token 余额。重置日期和时间以北京时间（UTC+8）显示。
