...
[Download the latest signed and notarized DMG](https://github.com/zuuzii-org/llm-pulse/releases/latest). LLM Pulse 2.3.0 requires macOS 14 or later and ships as a Universal App for Apple Silicon and Intel Macs.

The v2.3.0 release assets are:

- `LLM-Pulse-2.3.0.dmg`
- `LLM-Pulse-2.3.0.dmg.sha256`

Place both files in the same folder and verify the download with:

```bash
shasum -a 256 -c LLM-Pulse-2.3.0.dmg.sha256
```

Users on v1.0.0 must install v1.1.0 manually once because v1.0.0 did not include in-app updates. Users on v1.1–v1.3 should install and launch v1.4 before updating to v2.3.0.
